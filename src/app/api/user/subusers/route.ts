...
// GET /api/user/subusers — list all sub-users for current user
export async function GET(request: NextRequest) {
  return withUserSession(request, async (session) => {
    await connectDB();
    const users = await User.find({ parentId: session.sub })
      .select("-passwordHash")
...
export async function POST(request: NextRequest) {
  return withUserSession(request, async (session) => {
    const { email, password, displayName, phoneNumber, pin, username } = await request.json();
    ...
    await writeLog({
      ...
      entityId: uid,
      meta: { targetUid: uid, email: normalizedEmail },
    });
