...
export default function UserServicesPage() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const [provData, svcData] = await Promise.all([provRes.json(), svcRes.json()]);
  ...
  const { grouped, unassigned, activeProviders } = useMemo(() => {
    const grouped: Record<string, Service[]> = {};
    const unassigned: Service[] = [];
    const lookup = new Set(providers.map((p) => p.id));

    services.forEach((svc) => {
      const key = svc.providerId;
    ...
    const activeProviders = providers.filter((p) => (grouped[p.id] || []).length > 0);
    return { grouped, unassigned, activeProviders };
  }, [services, providers]);
  ...
  const isUnassignedSelected = selectedKey === "__unassigned__";
  const currentProvider = !isUnassignedSelected && selectedKey
    ? providers.find((p) => p.id === selectedKey)
    : null;
    ...
    <div className="p-6 sm:p-10 max-w-5xl mx-auto space-y-4 pb-12">
      ...
      <section className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-on-surface-variant font-body text-lg">Choose a provider to see the available services.</p>
        </div>
      </section>
      ...
      {services.length === 0 ? (
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
          {activeProviders.map((p) => {
            ...
            return (
              <button key={p.id} onClick={() => setSelectedKey(p.id)}
                ...
                <div className="flex-1 flex flex-col items-center justify-center p-4">
                  {p.logo ? (
                    <div className="w-16 h-16 rounded-2xl bg-primary/5 flex items-center justify-center overflow-hidden mb-3">
                      {isUrl ? (
                        <img src={p.logo} alt={p.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform" />
                      ) : (
                    ...
                    <div className="w-16 h-16 rounded-2xl bg-surface-container flex items-center justify-center mb-3">
                      <Signal className="w-6 h-6 text-on-surface-variant group-hover:text-primary transition-colors" />
                    </div>
                  )}
                  <h3 className="font-headline font-bold text-[#134235] text-sm text-center leading-tight group-hover:text-primary transition-colors">{p.name}</h3>
          ...
          {unassigned.length > 0 && (
            <button onClick={() => setSelectedKey("__unassigned__")}
              className="group relative bg-white border border-black/5 rounded-2xl overflow-hidden premium-shadow card-hover transition-all duration-200 aspect-[5/4] flex flex-col text-left">
              <div className="flex-1 flex flex-col items-center justify-center p-4">
                <div className="w-16 h-16 rounded-2xl bg-surface-container flex items-center justify-center mb-3">
                  <Zap className="w-6 h-6 text-on-surface-variant group-hover:text-primary transition-colors" />
                </div>
                <h3 className="font-headline font-bold text-[#134235] text-sm text-center leading-tight group-hover:text-primary transition-colors">Other Services</h3>
                <p className="text-xs text-on-surface-variant mt-1">{unassigned.length} service{unassigned.length !== 1 ? "s" : ""}</p>
              </div>
            </button>
          )}
        </div>
      ) : (
        /* ── SHOW SERVICES FOR SELECTED PROVIDER ── */
        <div>
          <button
            onClick={() => setSelectedKey(null)}
            className="flex items-center gap-2 text-sm font-semibold text-primary mb-6 hover:text-primary/80 transition-colors">
            <span>&larr;</span> Back to Providers
          </button>
          ...
          <div className="flex items-center gap-3 mb-5 pb-3 border-b border-outline-variant/30">
            {isUnassignedSelected ? (
              <>
                ...
                <h2 className="font-headline text-2xl font-bold text-[#134235]">Other Services</h2>
              </>
            ) : currentProvider && (
              <>
                {currentProvider.logo ? (
                  <div className="w-10 h-10 rounded-xl bg-primary/10 flex items-center justify-center overflow-hidden shrink-0">
                    {currentProvider.logo.startsWith("http") ? (
                      <img src={currentProvider.logo} alt={currentProvider.name} className="w-full h-full object-cover" />
                    ) : (
                  ...
                  <div className="w-10 h-10 rounded-xl bg-surface-container flex items-center justify-center">
                    <Signal className="w-5 h-5 text-on-surface-variant" />
                  </div>
                )}
                <h2 className="font-headline text-2xl font-bold text-[#134235]">{currentProvider.name}</h2>
              </>
            )}
          </div>

          {isUnassignedSelected ? (
            <RenderServiceList svcList={unassigned} />
          ) : currentProvider ? (
            <RenderServiceList svcList={grouped[currentProvider.id] || []} />
          ) : null}
        </div>
      )}
    </div>
