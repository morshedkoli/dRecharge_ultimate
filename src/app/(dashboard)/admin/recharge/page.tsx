...
export default function AdminRechargePage() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      console.error("Admin recharge page fetch error:", err);
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const RenderServiceList = ({ svcList }: { svcList: Service[] }) => {
    ...
    return (
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 mt-6">
        {svcList.map((svc) => (
          <Link key={svc.id} href={`/admin/recharge/${svc.id}`}
  ...
  return (
    <div className="p-6 sm:p-10 max-w-5xl mx-auto space-y-4 pb-12">
      <section className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-8">
        <div>
          <h1 className="font-headline text-4xl font-extrabold tracking-tight text-on-surface mb-2">Recharge</h1>
          <p className="text-on-surface-variant font-body text-lg">Select a service to initiate a transaction.</p>
        </div>
      </section>
      ...
      {services.length === 0 ? (
        <div className="border-2 border-dashed border-outline-variant rounded-2xl px-6 py-16 text-center">
          ...
          <p className="text-on-surface font-bold font-manrope text-lg mb-1">No active services</p>
          <p className="text-sm text-on-surface-variant">Enable services in the Services management page.</p>
        </div>
      ) : (
        <>
