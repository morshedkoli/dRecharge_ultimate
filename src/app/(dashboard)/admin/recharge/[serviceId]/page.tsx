...
export default function AdminRechargeServicePage({ params }: { params: Promise<{ serviceId: string }> }) {
  ...
  const fetchService = useCallback(async () => {
    try {
      ...
      if (res.status === 404) { setError("Service not found or no longer active."); return; }
      const data = await res.json();
      const svc = data.service as Service;
      if (!svc?.isActive) setError("This service is currently disabled.");
      setService(svc);
    } catch (err) {
    ...
    return (
      <div className="max-w-2xl mx-auto p-10 text-center bg-white rounded-3xl premium-shadow border border-red-100">
        ...
        <Link href="/admin/recharge"
          className="inline-flex items-center text-sm font-bold text-primary hover:text-primary-dim bg-[#E8F1EE] px-6 py-3 rounded-xl transition-colors font-manrope">
  ...
  return (
    <div className="max-w-4xl mx-auto pb-12">
      ...
      <div className="mb-8">
        <Link href="/admin/recharge"
          className="inline-flex items-center text-sm font-bold font-manrope text-on-surface-variant hover:text-[#134235] mb-6 transition-colors group">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-5 bg-white p-6 rounded-3xl border border-black/5 premium-shadow">
          ...
          <div className="pt-1">
            ...
            <p className="text-sm text-on-surface-variant font-medium leading-relaxed max-w-xl">
              {service.description || "Initiate a recharge or send money via this service."}
            </p>
          </div>
        </div>
      </div>
          ...
          <div className="bg-white rounded-3xl border border-black/5 premium-shadow p-6 sm:p-8">
            ...
            <form onSubmit={handleFormSubmit} className="space-y-6">
              <div>
                ...
                {recipient.length > 0 && recipient.length !== requiredRecipientLength && (
                  <p className="text-amber-600 text-xs mt-2 flex items-center font-bold font-manrope">
                    ...
                    Must be exactly {requiredRecipientLength} digits.
                  </p>
                )}
              </div>
              ...
              <div className="pt-6">
                ...
                >
                  {submitting ? (
                    <><Loader2 className="w-5 h-5 animate-spin text-white/70" /> Processing…</>
                  ) : (
                    <>Proceed <Send className="w-5 h-5" /></>
                  )}
                </button>
              </div>
            </form>
        ...
        <div className="md:col-span-2 space-y-6">
          ...
          <div className="bg-white border border-black/5 rounded-3xl p-6 premium-shadow">
            <h3 className="text-sm font-bold text-[#134235] mb-4 flex items-center gap-2 font-manrope">
              <Info className="w-4 h-4 text-primary" /> Note
            </h3>
            <ul className="text-xs text-on-surface-variant space-y-3 list-disc pl-4 leading-relaxed marker:text-black/10">
              <li>Deducted from your admin wallet immediately.</li>
              <li>Job queues for agent execution — completes within seconds.</li>
              <li>Balance refunded automatically if transaction fails.</li>
            </ul>
          </div>
        </div>
      </div>
          ...
          <div ref={containerRef} className="relative bg-white rounded-3xl premium-shadow w-full max-w-sm overflow-hidden animate-in fade-in zoom-in-95 duration-200 border border-black/5">
            <div className="flex items-center justify-between px-6 py-5 border-b border-black/[0.04]">
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-lg font-bold text-[#134235] font-manrope">Confirm Transaction</h2>
              </div>
            ...
            <form onSubmit={handleConfirmTransaction} className="p-6">
              ...
              <div className="mb-8">
                <label className="block text-[10px] font-bold uppercase tracking-widest text-on-surface-variant font-manrope mb-2 text-center">
                ...
                {pinError && (
                  <p className="text-red-500 text-xs font-bold mt-3 text-center flex items-center justify-center gap-1 font-manrope">
